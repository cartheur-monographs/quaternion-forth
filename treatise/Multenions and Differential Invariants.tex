...
\begin{center}
  {\Large\bfseries Multenions and Differential Invariants.\par}
\end{center}
\begin{center}\textsc{Alexander McAulay}\end{center}
...
$t$ ($=$ time), $x$, $y$, $z$, $\mathrm{J}$ ($=\sqrt{-1}$), $\theta$
($=$ arc), $\pi$.
...
Vectors ($V_1$): $\alpha$, $\beta$, $\gamma$, $\delta$, $\epsilon$,
$\zeta$, $\xi$, $\eta$, $\lambda$, $\mu$, $\nu$, $\rho$, $\sigma$, $\tau$.

Linear functions: $\xi$, $\eta$, $\lambda$, $\mu$, $\nu$, $\varpi$,
$v$, $\phi$, $\chi$, $\psi$.

Vectoriums: $\breve{\alpha}$, $\breve{\epsilon}$, $\breve{\eta}$,
$\breve{l}$, $\breve{\sigma}$, $\breve{\beta}$, $\breve{\upsilon}$, $\breve{\omega}$.

Differential symbols: $\delta$, $\Delta$, $\mathrm{H}$, $\mathrm{L}$,
$d$, $d_\rho$, $d_{\breve{\rho}b}$, $d_{s\alpha}$, $d\alpha$, $d\breve{b}$.

Selective linities, etc.: $\mathrm{K}$, $\mathrm{P}$, $\mathrm{Q}$,
$\mathrm{R}$, $\mathrm{S}$, $\mathrm{T}$, $\mathrm{U}$, $\mathrm{V}$.

\textsc{Def. 6.} Complexes of vectors, hypervectors, multenions,
...
\begin{equation}\mathrm{J}=\sqrt{-1}.\tag{6}\end{equation}
...
\begin{equation}i_4=\mathrm{J}\iota,\tag{7}\end{equation}
and drop out of most of our subsequent work all reference to the original $i_4$ and $\mathrm{J}$, using $\iota$ instead with its property $\iota^2=1$, as opposed to $\iota^2=-1$. Questions in relativity are then treated by aid of the multiplicative combinations of such expressions as
\begin{equation}xi_1+yi_2+zi_3+ct\iota.\tag{8}\end{equation}
whereby, so long as all the scalars thereby introduced are real, we shall be dealing with real questions in relativity. The general multenion thus constructed contains 16 and not 32 independent real scalars. $\iota$ is for practical relativity purposes real. We shall call such a multenion system semi-real.

\indent Now return to the meaning above assigned to $K$.
\begin{align*}K(i_1i_2i_3i_4)&=i_1i_2i_3i_4,\\ \text{and also}\quad K(i_1i_2i_3\iota)&=i_1i_2i_3\iota,\\ \text{but whereas}\quad K'(i_1i_2i_3i_4)&=(i_1i_2i_3i_4)^{-1}.\tag{9}\end{align*}
it is not true that the corresponding equation holds when we replace $i_4$ by $\iota$. I have therefore refrained from making the definition of $K$ depend on such equations as (9). It is, nevertheless, a perfectly easy theorem to prove that
\begin{equation}K\iota=\iota^{-1}.\tag{10}\end{equation}
when $\iota$ is any product of our \emph{original} generators $i_1$, $i_2$, \ldots{} $i_n$. This ceases to be true when $\iota$ takes the place of $i_4$, simply because $\mathrm{J}^{-1}$ is $-\mathrm{J}$ and not $+\mathrm{J}$. It may be noted, however, that there is always a reproscriptive $K\phi=R$ to be found, which is such that for every semi-real vectorium $\breve{\iota}$, $R\breve{\iota}=\breve{\iota}^{-1}$.

Thus, suppose we replace $i_{a+1}$, \ldots{} $i_n$ by $\iota_{a+1}$, \ldots{} $\iota_n$, where
\begin{equation}i_c=\mathrm{J}\iota_c\quad\text{and therefore}\quad \iota_c^2=1,\qquad c=a+1,a+2,\ldots,n.\tag{11}\end{equation}
Then
...
An objection to doing so is that this is not Hamilton's meaning in the case of his biquaternion $q+\mathrm{J}r$. His tensor is then imaginary, namely, one of the values of $[(q+\mathrm{J}r)K(q+\mathrm{J}r)]^{\frac12}$. In our form below, this amounts to saying that the tensor consists of a $V_0$ part $+aV_4$ part.
...
\begin{equation}
\begin{aligned}
\Lambda^{(x)}(u\Lambda^{(y)}vw)&=(-1)^{y+bc}\Lambda^{(x)}(u\Lambda^{(y)}wv)\\
&=(-1)^{x+y+a(b+c)}\Lambda^{(x)}(\Lambda^{(y)}vw\cdot u)
=(-1)^{x+bc+ca+ab}\Lambda^{(x)}(\Lambda^{(y)}wv\cdot u).
\end{aligned}
\tag{3}
\end{equation}
