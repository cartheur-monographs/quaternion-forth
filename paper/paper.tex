...
The deployment method also matters to reproducibility. In the current repo, the
Volatco-facing source is treated as block-resident \texttt{polyForth} source
rather than as an ephemeral terminal transcript. A generated \texttt{VOLATCO}
project image reserves blocks $0$ through $9$, uses block $10$ as the
application load block, and places the quaternion source in blocks $11$ through
$30$. The intended operator action is therefore to enter \texttt{AFORTH} and
issue \texttt{10 LOAD}. This detail is important because it makes the
experiment reproducible in the same block-oriented style used by the surrounding
saneForth workflow.
...
The first hardware experiment should not yet be a timing experiment. It should
be a deployment and correctness experiment that validates the block-image path
itself. In practical terms, that means loading the generated project image,
entering \texttt{AFORTH}, issuing \texttt{10 LOAD}, and checking that both the
quaternion and matrix report words produce the same rotated vector
$ (10, -20, -30) $. This is not the paper's central result, but it is a
necessary methodological step. Without it, any later performance numbers would
rest on an uncertain loading procedure and could reasonably be challenged as
artifact-dependent.

A conventional reference platform may still be useful, but only as a secondary
calibration experiment. If included, it should use the repository's
\texttt{gforth} reference implementation on a machine such as a Raspberry Pi
and should be presented as appendix evidence about the qualitative
quaternion-versus-matrix tradeoff, not as the primary basis of the paper's
claim.

\todoitem{Insert the primary Volatco results table here with at least
quaternion rotation versus matrix-vector rotation, including iteration count,
elapsed time, and power measurement point. The Raspberry Pi appendix
calibration already exists and should remain secondary.}
...
\item a reproducibility protocol for Volatco measurements
\item a block-image deployment path for Volatco with an explicit load block
\item an appendix-grade Raspberry Pi reference-platform calibration run
\end{itemize}
...
\item completed hardware measurements from a real Volatco board
...
\item add measured Volatco benchmark results
\item interpret the Volatco results against both the matrix baseline and the
Raspberry Pi appendix calibration
...
behavior.
\item Run Experiment 0 and Experiment 1 on real Volatco hardware.
\item Add the primary Volatco tables for timing, power, and code size.
