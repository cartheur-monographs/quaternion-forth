...
\indent By exactly similar reasoning (a little complicated by the presence of $k$), we may treat of equations (11) and (8) of §8. For the sake of saving space, I merely state the results: $kv$, $kdb$, $kec$, and $kds_a$ are covariant; while $kdb$, $\tau$, $w$, $k^{-1}V_0\nabla(k\tau)$, and $k^{-1}V_1V_1\nabla(kw)$ are contravariant. $kdb$ is both covariant and contravariant: it is invariant.
...
\indent It is not difficult to show that $kv$ satisfies the test of (8) for covariance. If in (2) and (3) of §6 we read $\chi$ in place of $\phi$ we get theorems closely connected with multiplication and composition in the absolute differential calculus. Let $v_a,v_b$ be covariant multenions of homogeneities $a,b$. Then
...
is contravariant (multiplication).
If $u$ is contravariant (homogeneity $a$) and $v$ is covariant (homogeneity $b$), then
...
$\xi^{-1}$ is said to be contravariant. From (8) we at once get the associated transformation rules. From this, when $\xi$ is covariant, its conjugate $\xi'$ is also covariant, and therefore both the self-conjugate part and the skew part of $\xi$ are covariant. Similarly for $\xi^{-1}$ and contravariance. Also, when $\xi$ is self-conjugate, $\chi$ is self-conjugate. If $\xi$ is a covariant vector linity, the extended linity is also covariant. Also, in this case, the rotation-$V_2$ linity $V_2\xi\chi\xi'$ is covariant.

\indent Let $\sigma_1,\sigma_2,\ldots,\sigma_n$ be $n$ independent covariant vectors. [They might be taken as $\nabla h_1,\nabla h_2,\ldots,\nabla h_n$ where the Jacobian of the quantities $h$ does not vanish.] From the reciprocal relations it at once follows that the corresponding reciprocal vectors are $n$ independent contravariant vectors. Any covariant or contravariant vector can be expressed at once in terms of these and invariants. Also, from the $2^n$ vectoriums formed from each set,
