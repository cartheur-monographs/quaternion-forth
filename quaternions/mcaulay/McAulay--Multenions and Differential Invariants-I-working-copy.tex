...
\begin{equation}V_0\phi r=V_0qr,\quad\text{then}\quad p=q,\tag{16}\end{equation}
...
\indent When in a semi-real system $R$ is chosen so that $Rl=l^{-1}$ for every $l$, we have that if $q=\sum xl$ and $q'=\sum x'l$, then
\[V_0qRq'=\sum xx',\]
and in particular
\[V_0qRq=\sum x^2.\]

\noindent\textsc{\S 3. The Commutation Theorem.}---If $\breve a$ is of homogeneity $a$, and $\breve b$ of homogeneity $b$, and if there are exactly $c$ primitive vectors common to the $a$ vectors of $\breve a$ and the $b$ vectors of $\breve b$ it is easy to see that $\breve a\breve b$ is of homogeneity $a+b-2c$, and that $\breve a\breve b=(-1)^{ab-c}\breve b\breve a$. Hence
\begin{equation}V_{a+b-2x}uv=(-1)^{ab-x}V_{a+b-2x}vu.\tag{1}\end{equation}
[Obviously true when $u=\breve a$, $v=\breve b$ as we see by considering separately the two cases $x=c$, $x\ne c$. It is therefore true for $u$ and $v$. This is a case of proof by reduction to primitive units.]
